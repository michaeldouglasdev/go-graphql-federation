	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "Cart":
			...
			case "findCartByID":
				...
				err = ec.PopulateCartRequires(ctx, entity, rep)
				if err != nil {
					return fmt.Errorf(`populating requires for Entity "Cart": %w`, err)
				}
				list[idx[i]] = entity
				return nil
			}
		case "CartItem":
			resolverName, err := entityResolverNameForCartItem(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "CartItem": %w`, err)
			}
			switch resolverName {
			...
			case "findCartItemByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findCartItemByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindCartItemByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "CartItem": %w`, err)
				}
...
func entityResolverNameForCartItem(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findCartItemByID", nil
	}
	return "", fmt.Errorf("%w for CartItem", ErrTypeNotFound)
}
